...
if __name__ == '__main__':
    ancestor_data_path = "GCDdata/init.pkl"
    directory = "GCDdata/ga/v5"

    for generation in range(20):
        best_iteration = -1
        best_data_path = ""
        for child in range(10):
            new_data_path = f"{directory}/g{generation}c{child}.pkl"
            if generation==0:
                mutate(ancestor_data_path, new_data_path, 4351)
            else:
                mutate(ancestor_data_path, new_data_path, (20-generation)**2+10)
            ...
            if best_iteration==-1 or (stop_iteration!=-1 and stop_iteration<=best_iteration):
                ...
                write_log(directory, "")
                write_log(directory, f"best iteration: {best_iteration}, train: {best_data_path}")
                write_log(directory, "")
        ancestor_data_path = best_data_path
